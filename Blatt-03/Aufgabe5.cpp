...
int main(){
	...
	for (int  k = 0; k < (int) 1000; ++k)
	{
		...
		for (int j = 0; j < 6; ++j)
		{
			Bahn[k][j] = Runge_Kutta( (i) , Start_Ort , Start_Geschwindigkeit , Kepler , Breite)[j];
		}
	}
	...
	//Runge_Kutta( 150 , Start_Ort , Start_Geschwindigkeit , Kepler , Datei , 0.1);
	/*Start_Ort[0] = 1.15393 ;
	Start_Ort[1] = -0.623457 ;
	Start_Ort[2] = 0.0;

	Start_Geschwindigkeit[0] =   -0.0246166;
	Start_Geschwindigkeit[1] =  0.879897;
	Start_Geschwindigkeit[2] = 0.0;

	Runge_Kutta( 20 , Start_Ort , Start_Geschwindigkeit , Kepler , Datei , 0.1);*/

	//20	0.998525	0.00286298	0	-0.502804	1.00002	0	
	Datei.close();
